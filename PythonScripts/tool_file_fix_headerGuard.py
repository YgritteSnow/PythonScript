...
def fixPrefixByLines( filelines, prefixString ):
	...
	if len(filelines) and ( filelines[-1].startswith("#endif") or filelines[-1] == " " or filelines[-1] == '\n' ):filelines.pop(-1)
	if len(filelines) and ( filelines[-1].startswith("#endif") or filelines[-1] == " " or filelines[-1] == '\n' ):filelines.pop(-1)
	if len(filelines) and ( filelines[-1].startswith("#endif") or filelines[-1] == " " or filelines[-1] == '\n' ):filelines.pop(-1)
	filelines.append( "\n#endif " )
	...
	for filetree_name in filelist:
		...
		for file_node in file_tree.allFile():
			...
			try:
				...
				fixPrefixByLines( f_lines, filepath_2_headerString(file_node.fullDir) )
				filestr.close()
...
def fixMyLastErrByLines( filelines, prefixString ):
	# 上边的一不小心多加了好几个 endif，这个函数和下个函数用来去掉这个错误。。
	if len(filelines) == 0:
		pass
	else:
		old_len = -1
	...
	while filelines[-1].find("#endif ") != -1 :
		filelines[-1] = filelines[-1][:-7]#.replace("#endif #endif ", "#endif ")

def fixMyLastErr():
	# 上边的一不小心多加了好几个 endif，这个函数和上个函数用来去掉这个错误。。
	print 'Fix Start'
	...
	for filetree_name in filelist:
		...
		for file_node in file_tree.allFile():
			...
			try:
				...
				fixMyLastErrByLines( f_lines, filepath_2_headerString(file_node.fullDir) )
				filestr.close()
